<!-- translation-of: CONTEXT.md sha256:cff1b16204f65053 -->
...
- 已在独立发布工作区从 `origin/main` 重建并准备好 `v0.2.0-alpha.3`（`0.2.0a3`），同时保留
  不可变的 Alpha 2 公开发布记录。
...
- 初始发布 commit `5d97742` 已通过公开 CI `33707942357`：12 组 Python 3.10–3.13 ×
  Linux／macOS／Windows 任务及构建产物／安全冒烟全部成功。
...
提交并推送最终发布状态，让该准确 commit 再次通过公开跨平台 CI；之后创建注释 Tag 和非 latest
的 GitHub 预发布，并上传重新构建的资产。
