<!-- translation-of: UPDATE_MAP.md sha256:b2e55ae794e42730 -->
...
[v0.2.0-alpha.2 候选说明](docs/release-notes/v0.2.0-alpha.2.zh-CN.md)
...
公开的 `v0.2.0-alpha.1` 是第一个可安装的点名目标审查桥梁。本地 `v0.2.0-alpha.2` 候选新增严格
`codex-scan` 边界，只处理一份选中导出，并内嵌诚实的记录／覆盖元数据。它**不会**发现目标历史、
声称导出完整，也不代表 Alpha 2 已公开发布。
...
| “我希望 Codex 使用相关历史，不要让我重新讲一遍。” | Alpha 2 绑定一份显式导出并内嵌有界输入信封；Alpha 3 归一化支持的现代 Codex 事件并记录语义排除项。 | Alpha 2 已本地实现；Alpha 3 待办 |
...
| 第 2 天 | `v0.2.0-alpha.2` | **已本地实现；未发布。** `codex-scan` 绑定一份选中导出、准确摘要／大小、目标／窗口、记录核算和诚实的部分覆盖；目录、主目录／根范围、链接边界、硬链接、漂移和超限输入全部失败即停止。 |
| 第 3 天 | `v0.2.0-alpha.3` | 归一化支持的现代 Codex 导出：完成项顺序、snapshot 去重、半开窗口和 automation／delegation 结构化排除；用正负 fixtures 与一个真实 named-Skill 审查证明。 |
